...
The $MSB$ curve is situated above $MPB$, because a positive externality of consumption is manifest.
...
When left on its own, the market engenders a welfare loss equal to size $A$, occasioned by the positive consumption externality. 
In particular, the market equilibrium would rest at $Q_0$, which is inferior to $Q_{opt}$, the optimum quantity of News the Finnish market should provide.
...
This is favourable, as the welfare loss in this case is limited to area $B$, which is smaller than the previously discussed area $A$.
...
\newpage
\noindent {\footnotesize Final note, not part of the commentary:\\
It turns out that news is not a private good but a \textbf{quasi-public good} (non-rivalrous and excludable), so the D/S diagrams I have are not suitable to describe the situation.
I was not warned of this when selecting the article, and the SL Micro unit barely covers the behaviour of such goods.
I hope that this oversight will not detriment my grade too severely.}
