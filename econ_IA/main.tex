...
\noindent The budget cut is illustrated in \textbf{figure 2} as a reduction in the supply of news.
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.7]
      \begin{axis}[
        ...
        \addplot[domain = 0:10, restrict y to domain = 0:10, samples = 1000] {(200-x^2.71828)^0.367879};

        \draw[-Triangle] (2, 8) to (1.95, 7.2);
        \draw[-Triangle] (8, 2) to (7.2, 1.9);
        %\draw[-Triangle] (6, 6) to (5.5, 5.4);

        \draw[dashed] (0, 6.6) -- (6.3337, 6.6);
        \draw[dashed] (6.3337, 0) -- (6.3337, 6.6);
        \draw[dashed] (0, 4.18423) -- (6.3337, 4.18423);

        \node[below] at (5, 10) {\footnotesize *A change $>8.6\%$ is shown for clarity};
        \node[below] at (5, -1) {Quality of Daily News (Arbitrary Units)};
...
\textbf{Figure 3} shows the reduction in Yle's production possibilities, and the choice it must face between producing radio programming and maintaining the quality of news.
...
In 2023, the amount of radio programming produced is 82210 hours \footnote{\url{https://yle.fi/aihe/ylen-vuosi-2023}}
hello.
