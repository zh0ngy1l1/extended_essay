...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\textwidth]{lpro_autofit.png}
  \caption{Logger Pro Auto Fit Curve for 0.25g trial}
\end{figure}

The curve will be fit on the data between 120 kPa and 140 kPa.
This interpolation always uses well above 80 data points.
It avoids the beginning of the data collection which contains an unwanted artifact that results from connecting the valve.
Using the Auto Fit functionality also automatically gives the error radius for $k_\mathrm{app}$, which is the only error in dependent variables that needs to be considered.
...
Only the value of $k_\mathrm{app}$ is ultimately needed, but $P_\infty$ is a useful sanity check, to ensure no major systematic error is taking place.
