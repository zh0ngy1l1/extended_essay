...
\begin{problem}
  Show that $\displaystyle \frac{6}{\pi^2}<\varphi(n)\sigma(n) n^{-2}<1$ for all integers $n$ greater than 1.
\end{problem}

For simplicity, $f(n)$ will be used to denote $\varphi(n)\sigma(n) n^{-2}$ from this point onwards.
...
(5.11) looks very promising.
In fact, the bound $\sigma(n)\varphi(n) n^{-2}<1$ is now obvious:
every term in the product is strictly less than 1, so the whole product must be less than 1 too.

Yet often a term will be very close to 1.
