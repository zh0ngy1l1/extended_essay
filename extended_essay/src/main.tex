...
\subsubsection*{Proof of Lemma 6.}
...
popularized by the mathematics youtuber 3Blue1Brown.
...
\vspace{-1em}\begin{enumerate}[label=(5.\arabic*), start=15]
  ...
  \item We expand $\frac{1-1/p^2}{1}$ into a geometric series.
  \item Write without $\Sigma$-notation.
  \item Explained below
  \item Write with $\Sigma$-notation.
\end{enumerate}
The step between (5.17) and (5.18) feels like a jump.
But each term in the infinite sum in (5.19) is produced exactly once
...
In general, each term $\tfrac{1}{n^2}$ is produced exactly once, corresponding to the unique prime factorisation 
$n = \prod p^{e_p}$ with $\tfrac{1}{n^2} = \prod p^{-2e_p}$.
...
By Lemma 6 we have, for $n=\prod_{i=1}^k p_i^{\alpha_i}$,
\begin{equation}
  f(n)\geq \prod_{i=1}^k \frac{p_i^2-1}{p_i^2}
\end{equation}
Each factor $\dfrac{p^2-1}{p^2}$ is less than 1, so adding more such factors (i.e. extending the product to more primes) strictly decreases the value. Hence
\begin{equation}
  \prod_{i=1}^k \frac{p_i^2-1}{p_i^2}
  > \prod_{p\ \text{prime}} \frac{p^2-1}{p^2} = \left(\prod_{p\ \text{prime}} \frac{p^2}{p^2-1}\right)^{-1}
\end{equation}
By Lemma 7 the product on the right equals $(\frac{\pi^2}(6))^{-1}=\frac{6}{\pi^2}$, therefore
\[
  f(n) > \frac{6}{\pi^2}.
\]
The inequality is strict because equality would require $n$ to be divisible by every prime (so that the finite product already contains every factor),
which is impossible for a finite integer $n$.
\qed\vspace{1em}

Hence we deem \textbf{Problem 1} as solved.

\section{Answering Question 1.}
