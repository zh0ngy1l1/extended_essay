...
I hope the conclusion is that the geometric mean is the best (whether 'best' may refer to most close to the target or easiest to compute).
Then I will proceed to graph the function $f(x)=\sqrt{\varphi(n) \cdot \sigma(n)}$ in more detail, compute the lower and upper bounds, show that they're strict, and lastly explain why the patterns occur.
