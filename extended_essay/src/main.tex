...
To show that $\sigma$ is multiplicative, we use the following lemma:
\begin{lemma}
  For coprime positive integers $m, n$,
  each element in $\mathcal{D}_{mn}$ is a product of a 
...
\[d_m \mid d \mid mn\]
so 
and $d_m$ shares no prime divisors with $n$
...
Moreover, the process of separating the terms between case 1 and case 2 is unique, so the corresponding pair $d_m, d_n$ is unique.


Conversely, if $d_m$ and $d_n$ are positive divisors
of $m$ and $n$, respectively, then $d_m d_n$ is a
divisor of $mn$.
This shows that the pair of elements $d_m, d_n$ is unique; 
...
To show that $\varphi$ is multiplicative, we use the following lemma, which is a special case of the 
\textbf{Chinese Remainder Theorem}.
\begin{lemma}
  For coprime positive integers $m, n$, each nonnegative integer $x$ with $0\leq x < mn$
...
\begin{example}
  ...
  \end{aligned}
  \]
  We see that each pair of remainders is unique.
\end{example}




Suppose, for contradiction, that there exists anonther integer $k'$, greater than $k$, with $0 \leq k' < mn$, which also has remainders $k_m, k_n$ when divided by $m$ and $n$, respectively.
