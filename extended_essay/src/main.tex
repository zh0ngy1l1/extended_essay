...
\begin{theorem}[Fundamental Theorem of Arithmetic]
  Every positive integer has a unique prime factorization.\cite{apostol}
\end{theorem}
...
% \begin{example}
%   The set $\{1,2,4,7,8,11,13,14\}$ has size $\left|\{1,2,4,7,8,11,13,14\}\right|=8$.
...
\vspace{1em}\begin{definition}[Euler's Totient Function $\varphi$]
...
\begin{example}
  ...
  Since there are 4 such numbers, $\varphi(15)=4$.
\end{example}
...
\textbf{Figure 2} visualizes the scenario by plotting points $(n, f(n))$ (in blue) and the lower-- and upper bounds (in red) on the Cartesian plane.
...
To approach \textbf{Problem 1}, we must improve our understanding on the functions $\sigma$ and $\varphi$.
The following product forms for these functions in terms of the prime factorization of $n$ shall be derived.

\begin{lemma}
  For any positive integer $n$ with prime factorization $n = \prod_{i=1}^{k} p_i^{\alpha_i}$, it holds
  \begin{align}
...
Using \textbf{Lemma 2}, \textit{multiplicativity} can be established using \textbf{Definition 1}.
\begin{align}
...
out, one by one, to reach the form in (4.15):
\begin{align}
...
To obtain the desired form in (4.2), we only need to compute $\sigma(p^\alpha)$ when $p$ is prime and $\alpha$ is any positive integer.
This is easy since
\begin{equation}
...
But $\varphi$ is only interested in the integers which are coprime.
...
Both terms in the right hand side of (4.22) are divisible by $p$, so $x$ must be divisible by $p$ as well.
...
Recall that \textbf{Definition 2} defines $\varphi(n)$ as the number of positive integers $x\leq n$
which are coprime to $n$, while \textbf{Lemma 3} and \textbf{lemma 4} are concerned
...
Combining \textbf{Lemma 3} and \textbf{Lemma 4} shows that
...
\subsubsection*{Finishing the product form of $\sigma$}
Now, exactly like (4.10) -- (4.15) in the derivation of the product form of $\sigma$,
we deduce that for $n = \prod_{i=1}^{k}p_i^{\alpha_i}$ holds
\begin{equation}
  \varphi(n)=\prod_{i=1}^{k}\sigma(p_i^{\alpha_i})
\end{equation}
...
The task was to prove $\frac{\pi^2}{6}<f(n)<1$ for all integers $n>1$.
...
\subsubsection*{Proof of Lemma 5.}
...
\begin{align}
  n^{-2} &= \left(\prod_{i=1}^{k}p_i^{\alpha_i}\right)^{-2}\\
         ...
         &= \prod_{i=1}^{k}p_i^{-2\alpha_i}
\end{align}
Combine (5.3) and (5.6) and simplify:
\begin{align}
...
\vspace{1em}\begin{example}
  ...
  Since $2025 = 3^4\cdot 5^2$, we use the formula (5.11)
  \[f(2025)=\frac{(3^{4+1}-1)}{3^{4+1}}\frac{(5^{2+1}-1)}{5^{2+1}} = \frac{242}{243}\cdot\frac{124}{125}=\frac{30008}{30375}\]
  Verifying using the answers of \textbf{Example 8} and \textbf{Example 11}:
...
Actually, \textbf{Lemma 5} looks very promising in regards to solving \textbf{Problem 1}.
...
The task is to show that $f$ is never exceeded by $\frac{\pi^2}{6}$, so we are interested
in how small $f$ can get.

To minimize the product $f(n) = \prod_{i=1}^{k} \frac{p_i^{\alpha_i+1}-1}{p_i^{\alpha_i+1}}$,
we have to 
\begin{enumerate}
  \item minimize each term in the product, and
  \item maximize the number of terms. Since each term is less than $1$, adding more terms decreases the product.
\end{enumerate}

\subsubsection*{Minimizing each term}
\begin{equation}
  \frac{p_i^{\alpha_i+1}-1}{p_i^{\alpha_i+1}} = 1 - \frac{1}{p_i^{\alpha_i+1}}
\end{equation}
From (5.13) it is clear that the larger $\alpha_i$ is, the larger the result will be.
In other words, minimizing the term is simply minimizing $\alpha_i$.
Since $\alpha_i$ is a positive integer, it is $1$ at minimum, in which case the term
would be $\frac{p_i^2-1}{p_i^2}$.

\vspace{1em}\begin{example}
  We find an integer $n$ such that $f(n) < f(2025)$ with this method:

  The prime factorization is $2025 = 3^4 5^2$.
  To minimize each term, we reduce the exponents to $1$.
  Therefore, choose $n = 3^1 5^1 = 15$.
  Then 
  \[f(n) = f(15)= \frac{8}{9}\cdot\frac{24}{25}=\frac{64}{75}<\frac{30008}{30375}=f(2025)\]
\end{example}\vspace{1em}

Therefore, to find the lower bound of $f$, we only need to consider the values $f(n)$ where each prime divisor of $n$
is only raised to the first power in the prime factorization.
In other words, we have
\begin{equation}
  f(n)\geq \prod_{i=1}^{k} \frac{p_i^2-1}{p_i^2}
\end{equation}
and equality holds when $\alpha_1 = \alpha_2 = \dots = \alpha_k$ in the prime factorization
$n = \prod_{i=1}^{k} p_i^{\alpha_i}$.

\subsubsection*{Maximizing number of terms}

Since each term $\frac{p_i^2-1}{p_i^2}$ is less than one, the more
terms we have, the smaller $f$ is.
Therefore, 
\[f(n) \geq \prod_{i=1}^{k} \frac{p_i^2-1}{p_i^2} > \prod_{p\ \text{prime}} \frac{p^2-1}{p^2}\]
where the right product is infinite, and goes over all primes $p$.
This works because each term $\frac{p_i^2-1}{p_i^2}$ in the left product is also a term in the right product,
and the right product always has more terms, because it is infinite while the left product is finite.

\subsubsection*{Evaluating the product over all primes}
Now all that's left to show the lower bound is to show that $\displaystyle \prod_{p\ \text{prime}} \frac{p^2-1}{p^2} = \frac{\pi^2}{6}$.
\begin{align}
  \prod_{p\ \text{prime}} \frac{p^2-1}{p^2} =& \prod_{p\ \text{prime}} \frac{1-\frac{1}{p^2}}{1}\\
  =& \prod_{p\ \text{prime}} \left(\left(\frac{1}{p^2}\right)^0 + \left(\frac{1}{p^2}\right)^1 + \left(\frac{1}{p^2}\right)^2 + \dots\right)\\
  =& \prod_{p\ \text{prime}} \left(\sum_{i=0}^{\infty} \frac{1}{p^{2i}}\right)
\end{align}
\vspace{-1em}\begin{enumerate}[label=(4.\arabic*), start=6]
  \item This is \textbf{Definition 1}.
  \item By \textbf{Lemma 2}, divisors of $mn$ correspond uniquely to products $d_m d_n$ with $d_m\mid m$, $d_n\mid n$, so the sum may be rewritten as a double sum over such pairs.
