...
The highlighted points in \textbf{\Cref{fig:2500_primes}} below seem to confirm our guess on why `dense lines' are formed.
...
\begin{definition}
  ...
  $n_1, n_2, \dots$
  form a `dense line at $y=t$' for some real number $t$ with
  $\frac{6}{\pi^2} \leq t \leq 1$ if the following conditions hold:
  \vspace{-1em}\begin{enumerate}
    \item Asymptotically, the \textit{density} of the sequence is some constant fraction of the density of primes.
    \item The sequence $f(n_1), f(n_2), \dots$ is increasing and approaches $t$.
  \end{enumerate}
\end{definition}

\noindent
The condition regarding density is 
Formally, for an increasing sequence of positive integers 
$a_1, a_2, \dots$
we define its \textit{asymptotic density} as the limit
\begin{equation}
  D = \lim_{x \to \infty} \frac{\left|\{a_i\ :\ i \in \mathbb{Z}^+,\ a_i \leq x\}\right|}{x},
\end{equation}
Verbosely, we would say that the density $D$ of the sequence is the 

\vspace{0.5em}
\noindent
We say that a sequence has \textit{at least the density of a constant multiple of the primes, asymptotically}, 
if there exists a constant $c>0$ such that
\[
  \#\{n_i \leq x\} \;\geq\; c \cdot \pi(x)
\]
for all sufficiently large $x$, where $\pi(x)$ denotes the number of primes not exceeding $x$.  
Equivalently, this means that the growth rate of the sequence, in terms of how many of its elements lie below $x$, is asymptotically no slower than a fixed positive multiple of that of the prime numbers.
